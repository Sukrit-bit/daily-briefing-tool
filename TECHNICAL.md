...
Read time is tier-aware: the estimate counts only the words actually rendered in the email per tier — deep dives include the full summary, insights, and so-what; worth-a-look includes the shorter summary and take; summary sufficient contributes almost nothing because only the take line renders. Items times 15 seconds scanning overhead plus rendered words divided by 200 WPM, clamped to a 3-minute minimum. This is more accurate than the V15 approach of summing all summary word counts regardless of how much each tier actually displays, and it's intentionally conservative — better to over-estimate than to set an expectation the reader can't meet.
...
Earlier versions used a headline index — a scannable list of all items at the top with tier emoji and truncated titles. User feedback showed this was consistently skipped: readers scrolled straight to the cards. V16 replaced the index with a compact header card showing tier count badges (e.g., "3 Deep Dives · 8 Worth a Look · 4 Summary Sufficient") and the editorial intro. The 30-second scan still works, but now it comes from the header card and badge counts rather than a line-item index.

The three tiers render as visually distinct card layouts, with decreasing visual weight as tier priority drops. This is a deliberate design decision: the card's appearance should signal how much attention it deserves before you read a word.

**Deep Dive** cards use a pink background, a 4px red left border, and full detail: 3-sentence summary, 3 key insights as bullet points, a blue "so what" callout box, and a Watch/Read link with duration. Deep dives also show a "concepts explained" glossary — up to 2 technical terms with plain-language explanations, rendered below the insights. These concept terms are user-facing text generated by the LLM, so they're HTML-escaped before rendering to prevent injection through crafted content. The concepts section only appears on deep dives because shorter-form content rarely introduces terminology that needs defining.

**Worth a Look** cards use a white background with a 3px yellow left border (intentionally narrower than deep dive's 4px — visual weight decreasing with priority). The summary is 2 sentences, insights are capped at 2 (reduced from deep dive's 3), and the opinion line renders as an inline italic "Take:" rather than a callout box. The Watch/Read link appears, but there's no concepts glossary. The structural difference from deep dive is the point: scanning the email, the background color, border weight, and presence or absence of the blue callout box immediately signal tier without reading the badge.

**Summary Sufficient** cards use a gray background with no left border, no link, no summary text, no insights, and no topic tags. Just the title, source, date, and an inline italic "Take:". The take IS the value — the editorial opinion is the only reason this item is in the briefing at all. Omitting the Watch/Read link is a product decision expressed through HTML: if the summary captures 90%+ of the content, a link to the original would imply the original is worth 15-40 minutes of the reader's time, contradicting the tier's own judgment. Content type badges ("Interview", "Market Call", "Analysis") appear in the meta line of all tiers, and items flagged as evergreen show an "Evergreen" badge — positioned as timeless content rather than old backlog.

The tier differentiation relies on `border-left` styling because it's one of the few CSS properties that renders consistently across email clients, including Outlook desktop (which uses Word's rendering engine). All styles are inline — no CSS classes, no `<style>` blocks — because most email clients strip them. The 24 tests covering these layouts validate that tier-specific content appears or is absent as expected: deep dives must have concepts and a callout box, worth-a-look must not have a callout box, summary sufficient must not have a link or summary text. The tests enforce the product rules through HTML structure assertions.

The link text adapts to content type: "Watch (42m)" for videos with known duration, "Watch" for videos without duration data, "Read" for articles.
