...
Delivery uses the Resend API, which has a sandbox limitation that cost debugging time: in sandbox mode, emails only arrive at the account owner's email address, regardless of what you set as the recipient. The API call succeeds and returns an ID, but the email never arrives at the intended address. There's no error, no bounce notification — the send looks perfectly successful from the application's perspective. This was discovered only after multiple "successful" sends that never arrived. Production delivery requires verifying a sending domain, which is a pending item.
